          ...
          rightAction={
            <div className="flex items-center gap-1">
              <button
                ...
                title={t.language === 'ta' ? 'கூகுள் ஷீட்ஸ் ஒத்திசைவு' : 'Sync Google Sheets'}
              >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5 select-none">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm flex flex-col justify-between">
              <div className="flex items-center justify-between text-slate-400">
                ...
                <span className="text-[8px] font-bold uppercase tracking-wider">{t.locations}</span>
              </div>
              <div className="mt-3">
                <div className="text-base font-extrabold text-slate-800 leading-none">
                  {metrics.totalPlaces}
                </div>
                <p className="text-[8px] text-slate-400 font-semibold mt-1">{t.deliveryPlacesSub}</p>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm flex flex-col justify-between">
              <div className="flex items-center justify-between text-slate-400">
                ...
                <span className="text-[8px] font-bold uppercase tracking-wider">{t.invoices}</span>
              </div>
              <div className="mt-3">
                <div className="text-base font-extrabold text-slate-800 leading-none">
                  {metrics.totalBills}
                </div>
                <p className="text-[8px] text-slate-400 font-semibold mt-1">{t.totalInvoicedSub}</p>
              </div>
            </div>
            ...
            <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 shadow-sm flex flex-col justify-between">
              <div className="flex items-center justify-between text-amber-600">
                ...
                <span className="text-[8px] font-bold uppercase tracking-wider">{t.pending}</span>
              </div>
              <div className="mt-3">
                <div className="text-sm font-black text-amber-800 leading-none">
                  {formatIndianCurrency(metrics.totalPendingAmount)}
                </div>
                <p className="text-[8px] text-amber-600/80 font-semibold mt-1">{t.outstandingBalanceSub}</p>
              </div>
            </div>
            ...
            <div className="bg-primary-50 border border-primary-100 rounded-2xl p-4 shadow-sm flex flex-col justify-between">
              <div className="flex items-center justify-between text-primary-600">
                ...
                <span className="text-[8px] font-bold uppercase tracking-wider">{t.collected}</span>
              </div>
              <div className="mt-3">
                <div className="text-sm font-black text-primary-800 leading-none">
                  {formatIndianCurrency(metrics.totalCompletedAmount)}
                </div>
                <p className="text-[8px] text-primary-600/80 font-semibold mt-1">{t.paidOffBalanceSub}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm space-y-3.5">
            <h3 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
              ...
              {t.topOutstandingLocations}
            </h3>

            {topOutstandingPlaces.length === 0 ? (
              <p className="text-[10px] text-slate-400 italic">
                {t.noOutstandingLocations}
              </p>
            ) : (
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-[8px] text-slate-400 font-semibold mt-0.5 uppercase tracking-wide">
                          {place.pending_count} {t.language === 'ta' ? 'நிலுவை பில்கள்' : 'pending invoices'}
                        </p>
                      </div>
                    </div>
                    ...
                    <div key={log.id} className="text-[10px] flex items-start justify-between gap-3 border-b border-slate-200/50 pb-2.5 last:border-0 last:pb-0">
                      <div className="space-y-0.5">
                        <span className="font-semibold text-slate-700 leading-snug">
                          {log.action_type === 'place_created' && t.place_created_log(log.place_name || '')}
                          {log.action_type === 'place_updated' && t.place_updated_log(log.place_name || '')}
                          {log.action_type === 'place_deleted' && t.place_deleted_log(log.place_name || '')}
                          {log.action_type === 'bill_created' && t.bill_created_log(log.bill_number || '', log.place_name || '')}
                          {log.action_type === 'bill_completed' && t.bill_completed_log(log.bill_number || '', log.place_name || '')}
                          {log.action_type === 'bill_updated' && t.bill_updated_log(log.bill_number || '', log.place_name || '')}
                          {log.action_type === 'bill_deleted' && t.bill_deleted_log(log.bill_number || '', log.place_name || '')}
                        </span>
                        {log.amount && (
